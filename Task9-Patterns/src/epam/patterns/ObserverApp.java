...
class MeteoStation implements Observed {
    int temperature;// 1.Наблюдаемый имеет и меняет свое состояние и посылает это изменение своим подписчикам,
    ...
    public void setMeasurements(int t, int p) { // 3.Наблюдаемый просто рассылает свое изменившееся состояние подписчикам
        temperature = t;
        pressure = p;
        notifyObservers();
    }
...
interface Observer {
    void handleEvent(int temp, int presser);// 4.Наблюдатели только могут получать сообщения, отправлять-не могут
